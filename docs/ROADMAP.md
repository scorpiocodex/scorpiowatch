...
- **`cwd` jailing** — resolve every Step's `cwd` against the configured project root and reject any that escapes it. Resolve *first*, then containment-check the **resolved** path, so a symlink pointing outward is caught rather than followed. Rejection happens at **config-validation time** as a `ConfigError` naming the offending trigger, step, and path — the same discipline that already rejects cyclic Workflows before they reach the `Scheduler` ([`EXECUTION_MODEL.md`](./EXECUTION_MODEL.md) §3) — so an escaping `cwd` fails at load with exit code `2` rather than mid-Run. [`SECURITY_MODEL.md`](./SECURITY_MODEL.md) §3 already documents this as a control, marked *(planned)*: **clear that marker when this lands.** Scheduled as a minor rather than a `v0.1.x` patch because it rejects configs that load successfully today, and a patch is the wrong place to start refusing input that previously ran. *(Raised by the post-release documentation-sync audit — the control was documented but never implemented; v0.1.0 passes `Step.cwd` to the child unvalidated.)*
- **Warn when a Step declares no `timeout_s`** — at config-validation time, name each such step and state that it will run unbounded. **Warning only — nothing is rejected**, so no existing config breaks. This is step one of the two-step migration toward requiring the field (step two in v0.3.0 below); shipping the warning a full minor ahead of the error means no user meets the requirement for the first time as a hard failure. *(Raised by the post-release documentation-sync audit.)*
...
- `swatch check` and `swatch doctor`
  - **Require `timeout_s` on every Step** — completes the migration begun in v0.2.0: the warning becomes a `ConfigError`, so no Workflow reaches the `Scheduler` carrying an unbounded Step. **Requiring the field is deliberately preferred over defaulting it.** A silent default is wrong for exactly the Steps that matter — too low and it kills a legitimately slow build, which surfaces as a flaky workflow rather than as the policy decision it actually was; too high and it protects nothing. Making the author name a number keeps the choice visible and keeps the failure mode "your config is incomplete" instead of "your build mysteriously died at 300s". This matches the project's standing posture of failing at load time with a pointed error rather than guessing. **Open question to settle when specified:** the explicit opt-out for a deliberately unbounded Step — today `timeout_s` is `float | None` with `gt=0`, so a sentinel (`timeout_s = 0`, or a distinct `unbounded = true`) has to be chosen rather than assumed. Clear [`SECURITY_MODEL.md`](./SECURITY_MODEL.md) §3's *(partially shipped)* marker when this lands. *(Raised by the post-release documentation-sync audit.)*
