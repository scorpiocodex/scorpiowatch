...
- **DevOps-critical capabilities move into the v1 band.** The daemon, per-component health endpoints, graceful shutdown, exit-code semantics, and environment-variable configuration land in **v1.1.0** (the DevOps unlock); the plugin host and the webhook source adapter in **v1.2.0**; metrics and container packaging in **v1.3.0**. The full re-sequencing is in [`ROADMAP.md`](./ROADMAP.md); capabilities displaced from the old v1 band (the storage query API, MCP client mode, richer `match` specs) are re-homed explicitly in the v2 band, and nothing is dropped.
