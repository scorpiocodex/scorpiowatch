...
**Context:** the configuration layer and the core needed the same domain types, with ownership split between them, so their imports crossed in both directions. The `pydantic` models that validate `watchflow.toml` — `Trigger`, `Workflow`, `Step`, and the top-level `WatchflowConfig` — were slated to live in `config/schema.py` ([`PROJECT_STRUCTURE.md`](./PROJECT_STRUCTURE.md) §1, [`MODULE_SPECIFICATIONS.md`](./MODULE_SPECIFICATIONS.md) §10), yet these are the core's own domain objects: the `TriggerEngine`, `Scheduler`, and `DAGExecutor` operate on them, and the `Engine` constructor takes a `WatchflowConfig` ([`MODULE_SPECIFICATIONS.md`](./MODULE_SPECIFICATIONS.md) §11). So `core/` had to import the models from `config/`, while `config/`'s loader had to import `core/` to return them — a `config → core → config` cycle. This is the same shape of problem as ADR-0010: two packages needing the same types with split ownership, which no single dependency direction — and therefore no single import-linter `layers` contract — can express.
**Decision:** `core/` owns the domain models. `Trigger` (and its `MatchSpec`) live in `core/triggers.py`, `Workflow` and `Step` in `core/workflow.py`, and `WatchflowConfig` in `core/config.py`. `config/` becomes a **pure loader**: `config/loader.py` reads and validates `watchflow.toml` and returns the core-owned `WatchflowConfig`, defining no domain models of its own. The dependency direction is one-way, `config → core`. This applies the same principle as ADR-0010 Option A — *the layer that defines the need owns the type*: the model, like the port, is owned by the core that consumes it, not by the peripheral package that merely constructs it. The model module `core/config.py` (`watchflow.core.config.WatchflowConfig`) and the loader package `watchflow.config` (`watchflow.config.loader.load`) are deliberately distinct — unambiguous to tooling, since they are different dotted paths, but surface-confusable to a reader skimming imports. That confusability is accepted rather than renamed away; the mitigation is a note in the `core/config.py` module docstring recording the distinction. It is recorded here to foreclose re-litigation of the name.
