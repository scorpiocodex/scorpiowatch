...
*Part of the WatchFlow documentation set — see [`WATCHFLOW.md`](./SCORPIOWATCH.md) [now: `SCORPIOWATCH.md`] for the full index. This is the amendment mechanism referenced in [`PROJECT_CONSTITUTION.md`](./PROJECT_CONSTITUTION.md) §7.*
...
> **Project rename — 2026-07-31.** The project was renamed from **WatchFlow** to
> **ScorpioWatch** before its first public release, because the PyPI distribution name
> `watchflow` was already taken. Three names changed, in three different directions:
>
> | What | Was | Now |
> |---|---|---|
> | Distribution (PyPI), repository, product name | `watchflow` / "WatchFlow" | `scorpiowatch` / "ScorpioWatch" |
> | Import package | `import watchflow` | `import swatch` |
> | CLI command | `watchflow run` | `swatch run` |
> | Config file | `watchflow.toml` | `swatch.toml` |
>
> The ADRs below are **left in their original wording**: where they say "WatchFlow,"
> `watchflow.toml`, `WatchflowConfig`, or `pip install watchflow`, that is the name the
> project had when the decision was made, and rewriting it would falsify the record.
> Where such a reference also describes something still true today, an inline
> `[now: …]` clarifier has been added beside it without altering the surrounding
> sentence. This rename changed no architecture and overturns no ADR.
...
**Consequences:** a fresh `pip install watchflow` [now: `pip install scorpiowatch`] opens no network listener and trusts no external broker by default — the safest possible out-of-the-box posture, at the cost of one extra install step for DevOps/CI use cases that need those sources.
...
**Status:** Accepted — 2026-07-25 (Option A; implemented in task 0.3)
...
- **ADR-0006 is not overturned.** Webhook, Queue, Git, and CI-provider adapters remain official first-party *plugins*, not core. A fresh `pip install watchflow` [now: `pip install scorpiowatch`] still opens no network listener and trusts no external broker by default. Where the re-baseline needs the webhook adapter earlier than its old v2.1.0 slot, it does so by **pulling the plugin host forward to v1.2.0** so the webhook adapter ships as a plugin there — it is *not* promoted into core to make the date. The safest-out-of-the-box posture ADR-0006 guarantees is preserved intact.
...
## ADR-0012 — Core owns the domain models; `config/` becomes a pure loader

**Status:** Accepted — **2026-07-25**
**Context:** the configuration layer and the core needed the same domain types, with ownership split between them, so their imports crossed in both directions. The `pydantic` models that validate `watchflow.toml` [now: `swatch.toml`] — `Trigger`, `Workflow`, `Step`, and the top-level `WatchflowConfig` [now: `SwatchConfig`] — were slated to live in `config/schema.py` ([`PROJECT_STRUCTURE.md`](./PROJECT_STRUCTURE.md) §1, [`MODULE_SPECIFICATIONS.md`](./MODULE_SPECIFICATIONS.md) §10), yet these are the core's own domain objects: the `TriggerEngine`, `Scheduler`, and `DAGExecutor` operate on them, and the `Engine` constructor takes a `WatchflowConfig` ([`MODULE_SPECIFICATIONS.md`](./MODULE_SPECIFICATIONS.md) §11). So `core/` had to import the models from `config/`, while `config/`'s loader had to import `core/` to return them — a `config → core → config` cycle. This is the same shape of problem as ADR-0010: two packages needing the same types with split ownership, which no single dependency direction — and therefore no single import-linter `layers` contract — can express.
**Decision:** `core/` owns the domain models. `Trigger` (and its `MatchSpec`) live in `core/triggers.py`, `Workflow` and `Step` in `core/workflow.py`, and `WatchflowConfig` in `core/config.py`. `config/` becomes a **pure loader**: `config/loader.py` reads and validates `watchflow.toml` and returns the core-owned `WatchflowConfig`, defining no domain models of its own. [Throughout this record, `WatchflowConfig` is now `SwatchConfig` and `watchflow.toml` is now `swatch.toml`.] The dependency direction is one-way, `config → core`. This applies the same principle as ADR-0010 Option A — *the layer that defines the need owns the type*: the model, like the port, is owned by the core that consumes it, not by the peripheral package that merely constructs it. The model module `core/config.py` (`watchflow.core.config.WatchflowConfig`) and the loader package `watchflow.config` (`watchflow.config.loader.load`) [now: `swatch.core.config.SwatchConfig` and `swatch.config.loader.load`] are deliberately distinct — unambiguous to tooling, since they are different dotted paths, but surface-confusable to a reader skimming imports. That confusability is accepted rather than renamed away; the mitigation is a note in the `core/config.py` module docstring recording the distinction. It is recorded here to foreclose re-litigation of the name.
**Consequences:** with `core/` importing nothing from `config/` — and, per ADR-0010 Option A, nothing from `adapters/` — the dependency graph became a strict total order, so the five hand-written `forbidden` import-linter contracts from task 0.2 collapsed into a single `layers` contract (see ADR-0010). `config/schema.py` is not created; `config/` holds only the loader. The prior statements that `core/` may depend on `config/` ([`PROJECT_STRUCTURE.md`](./PROJECT_STRUCTURE.md) §2) and that the config models live in `config/schema.py` ([`PROJECT_STRUCTURE.md`](./PROJECT_STRUCTURE.md) §1, [`MODULE_SPECIFICATIONS.md`](./MODULE_SPECIFICATIONS.md) §10) are now incorrect; reconciling them, together with the ADR-0010 Option A doc edits, is the documentation task that follows this record. No runtime behavior changed — this is a module-ownership and import-direction decision, taken while the models are still skeletons so its cost is at its minimum.
