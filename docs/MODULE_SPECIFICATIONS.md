...
**Cooldown — as built (v0.1.0, pulled forward from v0.3.0):** a **leading-edge** throttle keyed on `(trigger.name, matched_path)` — the path is the matched event's `payload["path"]`, so an editor's write-temp-then-rename collapses to the one stable post-rename path. The first fire for a key is admitted and opens a **fixed window measured from that first fire** (a `time.monotonic()` deadline); a subsequent fire for the same key while the window is live is **suppressed** — never a silent drop, but an observable admission decision logged as `admission.suppressed` (`reason`, `trigger`, `path`, `remaining_ms`) and surfaced to the output layer (visible under `--verbose` / `--json`, coalesced into the human summary). When the window elapses the next fire is admitted and opens a fresh window. **Configuration:** `Trigger.cooldown_ms` — `None` (unset) uses the Scheduler's default window (**300 ms** as shipped), `0` disables cooldown for that trigger, `N > 0` sets an N-ms window; on by default. The cooldown key→deadline map is bounded by **lazy purge-on-access** eviction (expired keys are dropped on admission once the map grows past a threshold; no background sweeper). This is distinct from **debounce** (trailing-edge coalescing of a spaced burst onto the settled state), which remains deferred — see [`ROADMAP.md`](./ROADMAP.md).
